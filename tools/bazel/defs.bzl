...
def expand_template_action(ctx, substitutions):
    ...
    output = ctx.actions.declare_file(ctx.file.template.path)
...
def expand_template_temporary_action(ctx, substitutions):
    """Writes a templated file with the given version.
...
def _expand_template_temporary_impl(ctx):
    """Writes a templated file with the given version.
...
expand_template_temporary = rule(
    attrs = {
        "substitutions": attr.string_dict(
            allow_empty = False,
            doc = "Substitutions to make when expanding the template.",
    ...
    This is a wrapper around actions.expand_template.
    """,
    implementation = _expand_template_temporary_impl,
    provides = [
        DefaultInfo,
    ],
)
